...
With 394.76 million tonnes of CO₂ emitted in 2021, Argentina accounts for approximately 0.83% of global emissions [@climatewatch2025]. Although not a major polluter, the government implemented a tax reform in 2017 that included the introduction of a carbon tax through the amendment of Law No. 23.966. As a result, Argentina is one of the few Latin American countries with a carbon tax, alongside Colombia, Chile, Mexico, and Uruguay. This reform aligns with a growing global trend toward carbon taxation.

Carbon taxes are increasingly seen as a crucial tool to drive environmental sustainability by reducing greenhouse gas emissions. There are indeed, a cost-effective means of reducing emissions compared to command-and-control regulations or subsidies [@gugler2021]. As a Pigouvian tax, carbon pricing helps internalize the external costs of carbon emissions, creating incentives for households to adjust their energy consumption behavior toward cleaner alternatives.
...
In 2017, Macri's government introduced a carbon tax, layered on top of an existing tax on liquid fuels. The carbon tax consists of fixed lump sums per unit volume of each fuel, adjusted for inflation every quarter and varying according to the carbon dioxide (CO₂) emissions associated with each fossil fuel. The government planned a gradual increase in the tax rate, an annual 10% hike from 2019 to 2028 [@méxico2-plataformamexicanadecarbono2023]. As of 2023, the carbon price resulting from these increases corresponded to approximately USD 10 per ton of CO₂ (USD 10/tCO₂) [@méxico2-plataformamexicanadecarbono2023]. Tax rates are relatively low compared to the what is advised by the IMF, which sets the floor for middle-income countries at USD 50/tCO₂ [@economiccommissionforlatinamericaandthecaribbean2025], but higher than in regional peers such as Chile, Colombia, and Mexico. The tax is applied upstream, targeting fuel importers and producers, rather than directly to households. Its design resembles that of a typical Pigouvian tax.

On the other hand, Canada has a dual carbon pricing system, consisting of a federal fuel charge and a separate system for large industrial emitters. The federal fuel charge is applied to producers, importers, and distributors of fossil fuels, as well as other CO₂-emitting entities such as air and marine carriers [@canadarevenueagency2022][.](https://www.canada.ca/en/revenue-agency/services/tax/excise-taxes-duties-levies/fuel-charge/registration.html)Although the charge is levied upstream, it is often passed on to consumers.

As of April 2025, the federal fuel charge has been discontinued, but its structure will inform the design of this microsimulation [@canadaenvironmentandclimatechange2018]. Like Argentina, Canada sets specific charges for specific fuel types [@canadadepartmentoffinance2025]. However, each province is allowed to implement a higher carbon price than the federal benchmark. Other countries, such as Mexico, also apply differentiated carbon taxes across regions.
...
-   Use microdata from Argentina's national household expenditure survey (Encuesta Nacional de Gastos de los Hogares, ENGHo) to estimate baseline household spending on energy sources [@indec:institutonacionaldeestadísticaycensosdelarepúblicaargentina2018].
...
Building on our third model, we will examine how higher carbon prices reshape household demand for key energy sources. [@zabaloy2022] report short-run price elasticities ranging from –0.09 (highly inelastic) to –0.76 (more responsive). We adopt the upper-level magnitude of –0.76 for two reasons. First, several household fuels in the study—notably firewood, kerosene, and bottled gas—cluster near this more elastic estimate, so –0.76 offers a realistic benchmark for those segments of Argentina’s energy mix that can, in practice, be switched or curtailed. Second, using the most elastic value gives us an upper-bound scenario: it reveals the maximum plausible demand response and helps policymakers gauge the ceiling on emissions reductions, revenue volatility, and distributional impacts if households react more aggressively than the historical average. Third, the –0.76 elasticity is forward-looking.
...
```{r}
elasticity <- -0.76   # Change for different scenarios
...
**Household percent change in net income after tax**
...
By imposing a 50% surcharge on fossil fuels, the dirtiest energy sources, kerosene and coal, are hit hardest, while a modest 10% levy on gas only gently nudges users toward cleaner fuel. With the elasticity, this mix generates strong consumption shifts away from taxed fuels and toward subsidized or cleaner alternatives. By forgoing any rebates, the policy keeps the spotlight squarely on pure price signals to drive behavioral change and technology adoption.
...
A bolder tax increase (+30%) across all polluting fuels to drive stronger carbon cuts, paired with a large targeted rebate (40% of revenues) for the poorest 40% to protect equity.
...
This analysis simulates how total carbon tax revenue changes with increasing tax rates, using three demand elasticities: -0.9, -1.0, and -1.1. Tax increases range from 0% to 20,000%. For each tax level, demand is adjusted based on elasticity, and new revenues are calculated accordingly.

The first graph shows that total revenue rises with higher taxes but at a slowing pace. Lower elasticity values (e.g., -0.9) result in higher revenue, as demand falls less sharply. The second graph, which shows the slope of the revenue curve, highlights that the revenue gain per unit of tax increase drops sharply after the initial stages. This suggests diminishing returns from further tax hikes, with revenue gains flattening at high tax levels.
...
# Define elasticities and tax multipliers
elasticities <- c(-0.76, -1, -1.1)
tax_multipliers <- seq(0, 200, by = 5)

# Grid of combinations
simulation_grid <- expand.grid(elasticity = elasticities, tax_multiplier = tax_multipliers)

# Run simulations
laffer_data <- simulation_grid %>%
  mutate(
    total_revenue = future_map2_dbl(elasticity, tax_multiplier, function(e_val, multiplier) {
      
      carbon_tax_temp <- carbon_tax %>%
        mutate(
          carbon_tax_item = map2_dbl(
            cantidad, articulo,
            ~ calculate_carbon_tax(.x, .y,
                                   fossil = multiplier,
                                   gas = multiplier,
                                   wood = multiplier)
          ),
          ...
          cantidad2 = cantidad * (1 + tax_rate) ^ e_val,
          price2 = price * (1 + tax_rate),
          carbon_tax_item2 = cantidad2 * tax_per_unit
        )
      ...
      carbon_tax_temp %>%
        group_by(id) %>%
        summarise(tax_paid = sum(carbon_tax_item2, na.rm = TRUE), .groups = "drop") %>%
        summarise(total = sum(tax_paid, na.rm = TRUE)) %>%
        pull(total)
    })
  )

ggplot(laffer_data, aes(x = tax_multiplier * 100, y = total_revenue, color = factor(elasticity))) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Laffer Curve: Carbon Tax Revenue vs. Tax Increase (%)",
    x = "Tax Increase (%)",
    y = "Total Carbon Tax Revenue",
    color = "Elasticity"
  ) +
  theme_minimal()


laffer_slopes <- laffer_data %>%
  arrange(elasticity, tax_multiplier) %>%
  group_by(elasticity) %>%
  mutate(
    slope = c(NA, diff(total_revenue) / diff(tax_multiplier))
  ) %>%
  ungroup()

ggplot(laffer_slopes, aes(x = tax_multiplier * 100, y = slope, color = factor(elasticity))) +
  geom_line(size = 1.2) +
  geom_point(size = 2) +
  labs(
    title = "Slope of Laffer Curve (Δ Revenue / Δ Tax)",
    x = "Tax Increase (%)",
    y = "Revenue Slope",
    color = "Elasticity"
  ) +
