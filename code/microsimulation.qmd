...
# How much the spend in energy before and after as a proportion of income by quintile (i dont like this one)
income_plot <- ggplot(plot_data, aes(x = factor(quintile), y = value, fill = income_type)) +
...
# Relative Tax Burden
tax_burden_plot <- ggplot(quintile_averages, aes(x = factor(quintile), y = tax_percentage)) +
...
tax_share_plot <- ggplot(tax_by_quintile, aes(x = factor(quintile), y = tax_share)) +
  geom_bar(stat = "identity", fill = "purple") +
  labs(
    title = "Participación en la recaudación del impuesto al carbono por quintil",
    x = "Quintil de ingreso",
    y = "Porcentaje de la recaudación total (%)"
  ) +
  theme_minimal()
...
ggplot(quintile_contributions, aes(x = factor(quintile), y = percent_contribution)) +
  geom_bar(stat = "identity", fill = "purple") +
  labs(
    title = "Carbon Tax Revenue Contribution by Income Quintile",
    x = "Income Quintile",
...
calculate_carbon_tax <- function(cantidad, articulo) {
  ...
  tax_rates <- case_when(
    ...
    articulo == "A0453101" ~ 3.54,  # Kerosene 
    articulo == "A0453103" ~ 3.02,  # Otros combustibles 
    
    # Gas (emisiones medias)
    articulo == "A0452101" ~ 2.12,  # Gas envasado en garrafas 
    articulo == "A0452102" ~ 2.12,  # Gas a granel
    articulo == "A0452103" ~ 2.61,  # Gas natural - vivienda principal
    articulo == "A0452104" ~ 2.12,  # Gas envasado en tubo
    articulo == "A0452105" ~ 2.61,  # Gas natural - vivienda secundaria 
    
    # Electricidad (varía según matriz energética, pero es menos contaminante directamente)
    articulo == "A0451101" ~ 0,  # Electricidad - vivienda principal
    articulo == "A0451102" ~ 0,  # Electricidad - vivienda secundaria
    articulo == "A0451103" ~ 0,  # Gastos de electricidad para otra vivienda
    
    # Leña y carbón (puede ser renovable o no dependiendo de la fuente)
    articulo == "A0453102" ~ 0,  # Leña y carbón 
    
    # Otros
    TRUE ~ 0                     # Tasa estándar 
  )
...
quintile_analysis <- household_tax %>%
  ...
  summarise(
    ...
    avg_net_income = mean(net_income, na.rm = TRUE),
    avg_tax = mean(total_carbon_tax, na.rm = TRUE),
    avg_spent_ene_before = mean(total_spent_energy, na.rm = TRUE),
    avg_spent_ene_after = avg_spent_ene_before+ avg_tax,
    avg_tax_burden = mean(total_carbon_tax/ingtoth * 100, na.rm = TRUE) 
  )

# Step 5: Analysis per quintile and type of Energy (joining with original information)
carbon_tax_with_quintile <- carbon_tax %>%
  left_join(
    household_tax %>% select(id, quintile, total_carbon_tax), 
    by = "id"
...
# Step 6: Calculate Total Carbon Tax Revenue
total_revenue <- sum(household_tax$total_carbon_tax, na.rm = TRUE)


# Step 7: Step 7: Calculate Share of Revenue per Quintile
quintile_contributions <- household_tax %>%
  group_by(quintile) %>%
  summarise(
    total_tax_paid = sum(total_carbon_tax, na.rm = TRUE)
...
# Now we can run the analysis per quintile and type of energy 
quintile_fuel_analysis <- carbon_tax_with_quintile %>%
  group_by(quintile, articulo_label) %>%
  summarise(
    avg_carbon_tax = mean(carbon_tax_item, na.rm = TRUE),
    avg_income_aft_tax = mean(),
    avg_tax_rate = mean(tax_rate * 100, na.rm = TRUE), #Average tax rate in %
    count = n()  # Nb of households that use this source
  )


#Graph 1: How much the spend in energy before and after as a proportion of income by quintile

energy_exp_comparison <- quintile_analysis %>%
  select(quintile, avg_spent_ene_before , avg_spent_ene_after) %>%
  pivot_longer(
    cols = c(avg_spent_ene_before, avg_spent_ene_after),
    names_to = "energy_exp",
    values_to = "amount"
  )

energy_exp_plot <- ggplot(
 energy_exp_comparison,
  aes(x = factor(quintile), y = amount, fill = energy_exp)
) +
  geom_bar(stat = "identity", position = position_dodge()) +
  scale_fill_manual(
  values = c("avg_spent_ene_before" = "red", "avg_spent_ene_after" = "blue"),
  labels = c("avg_spent_ene_before" = "Energy Expenditure Before Tax",
             "avg_spent_ene_after" = "Energy Expenditure After Tax")
) +
  labs(
    title = "Differences in Energy Expenditure per Income Quintile",
    x = "Income Quintile",
    y = "Energy Expenditure per Household",
    fill = ""
  ) +
  theme_minimal()

print(energy_exp_plot)
...
ggplot(quintile_contributions, aes(x = factor(quintile), y = percent_contribution)) +
  ...
  labs(
    title = "Share of Carbon Tax Revenue by Income Quintile",
    x = "Income Quintile",
    y = "Share of Total Revenue (%)"
  ) +
  theme_minimal()
